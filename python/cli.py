...
def main(address):
    ...
    while True:
        ...
        # Ok send them from the current user's account (which is?)
        # to the newly-created account
        jobcoin_client = JobcoinClient(address)

        successes, failures = jobcoin_client.send_coins(
            [deposit_address], amount)
        if failures:
            failure = failures[0]
            click.echo("Failed to send {0} coins to {1}. "
                       "Please try again.\n\n".format(
                        failure.amount, failure.address))
            continue
        click.echo(
            '\nJust sent {amount} jobcoins to {deposit_address}. '
            'Please standby for disbursement.....'
            '\nThere will be a standard {disbursement_fee_percent} '
            'percent fee.\n'.format(
                amount=amount, deposit_address=deposit_address,
                disbursement_fee_percent=JobcoinClient.DISBURSEMENT_FEE_PERCENT
            ))

        # Then do the transfer from the parent account to the
        # supplied accounts (w/a little randomization)
        disbursement_client = JobcoinClient(deposit_address)

        successes, failures = disbursement_client.disburse_funds(
            addresses, amount)
        if len(failures):
            click.echo("Did not distribute all of the funds!"
                       "Please retry the following transactions:")
            for txn in failures:
                print(txn)
            continue

        total_new_funds_available = sum([x.amount for x in successes])
        click.echo("Please expect {0} jobcoins"
                   " to be available across accounts {1}.".format(
