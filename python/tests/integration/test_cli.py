...
def test_cli_basic():
    ...
    assert 'Welcome, {0} to the Jobcoin mixer'.format(input_address) in result.output
...
def test_cli_creates_address():
    runner = CliRunner()
    address_create_output = runner.invoke(cli.main, args=["inputAddress"]).output
...
# This will need to somehow dependency-inject a mock client whose behavior we control
